(function(){
    var targetName = "Pin-Mutfa-Havlu";
    var campID = 253;
    var impSent = false;
    var productData = {boxBottom: "30px", boxRight: "0px", name:"%100 Pamuk Tribal 2'li Mutfak Havlusu", img: "https://cdn.bellamaison.com/tribal-mutfak-havlusu-siyah-mutfak-havlusu-bella-maison-15005-11-K.jpg", href: "https://www.bellamaison.com/tribal-mutfak-havlusu-siyah", sku: "2003MBEZ0005|102", dataPk: "", firstPrice: 79.95, finalPrice: 39.95};
    var iconData = {pinTop: "40%", pinRight: "40%", img:"https://img.euromsg.net/74C9BB1BBC5D4AB89DFCFDA808111C2E/images/plus.png", color: 'transparent'};
    var bannerSelector = ".col.col-6.p-right";
    ...
    jQuery(bannerSelector).eq(12).append(iconHtml);
    jQuery(bannerSelector).eq(12).css("position","relative");
    var productHTML = '<style class="vl-product-view-container-style" data-product-id="'+productData.sku+'">\
    .vl-product-view-container[data-product-id="'+productData.sku+'"]{z-index: 1000003!important;position: absolute;margin: auto; padding: 10px;transition: opacity 1.5s;background-color: white;border-radius: 10px;width: 400px;height: 213px;left: 0;right: '+productData.boxRight+';top: 0;bottom: '+productData.boxBottom+';-webkit-box-shadow: 0px 1px 31px 8px rgba(0,0,0,0.23);-moz-box-shadow: 0px 1px 31px 8px rgba(0,0,0,0.23);box-shadow: 0px 1px 31px 8px rgba(0,0,0,0.23);}\
    ...
    if(jQuery('.vl-product-view-container').length>0){
        jQuery('.vl-product-view-container').remove();
    }

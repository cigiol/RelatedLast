
if (document.cookie.indexOf(cappingName) === -1) {
if (VisiParameters["OM.pv"]) {
    if (VisiParameters["OM.pn"].includes("Wainer")) {
        var cValue = getCookie(targetName);
        if(cValue){
            if(cValue=="true"){
            setTimeout(() => {
                if (window.innerWidth > 500) {
                    showLightBoxDesktop("270px", "550px");
                }
                else{
                    showLightBoxMobile("auto", "280px");
                }
            }, 3000);
        }
        }
        
        SetCookie(targetName, 24);
        
        
    }
}

function getCookie(cname) {
    var name = cname + "=";
    var decodedCookie = decodeURIComponent(document.cookie);
    var ca = decodedCookie.split(';');
    for(var i = 0; i <ca.length; i++) {
      var c = ca[i];
      while (c.charAt(0) == ' ') {
        c = c.substring(1);
      }
      if (c.indexOf(name) == 0) {
        return c.substring(name.length, c.length);
      }
    }
    return "";
  }

if (document.cookie.indexOf(targetName) !== -1) {
    if (window.innerWidth > 500) {
        document.addEventListener("mouseleave", function (e) {
            if (e.clientY < 0 && exitCheck) {
                showLightBoxDesktop("270px", "550px");
                exitCheck = false;
            }
        }, false);
    }
    else {
        var checkScrollSpeed = (function (settings) {
            settings = settings || {};
            var lastPos, newPos, timer, delta,
                delay = settings.delay || 50;
            function clear() {
                lastPos = null;
                delta = 0;
            }
            clear();

            return function () {
                newPos = window.scrollY;
                if (lastPos != null) {
                    delta = newPos - lastPos;
                }
                lastPos = newPos;
                clearTimeout(timer);
                timer = setTimeout(clear, delay);
                return delta;
            };
        })();
        var lastScrollTop = 0;
        window.addEventListener("scroll", function () {


            var st = window.pageYOffset || document.documentElement.scrollTop;
            if (st > lastScrollTop) {
                console.log("down scrolling")
            } else {
                if (checkScrollSpeed() <= -80) {
                    if (exitCheck) {
                        showLightBoxMobile("auto", "280px");
                        exitCheck=false;
                    }
                }
            }
            lastScrollTop = st <= 0 ? 0 : st;

        }, false);
    }

}

var targetName = "wainer-exit";
var cappingName="wainer-capping";
var exitCheck = true;

function SetCookie(cookieName, hours) {
    var date = new Date();
    date.setTime(date.getTime() + hours * 1 * 60 * 60 * 1000);
    var cookieValue = getCookie(targetName);
    if(cookieValue){
        if(cookieValue=="false"){
            document.cookie = cookieName + "=true;expires=" + date + ";path=/";
        }
        else if(cookieValue=="true"){
            document.cookie = cookieName + "=enough;expires=" + date + ";path=/";
        }
    }
    else{
        document.cookie = cookieName + "=false;expires=" + date + ";path=/";
    }
    
}

}
function showLightBoxDesktop(height, width) {

    var visi = document.createElement("div");
    visi.setAttribute("id", "VisiNotifSection");


    var overlay = document.createElement("div");
    overlay.setAttribute("id", "visi_lightbox_overlay2_wrapper2");
    overlay.style.position = "absolute";
    overlay.style.top = "0";
    overlay.style.right = "0";
    overlay.style.left = "0";
    overlay.style.bottom = "0";


    overlay.style.zIndex = "1000003";
    var overlay_light = document.createElement("div");
    overlay_light.setAttribute("id", "visi_lightbox_overlay2");
    overlay_light.style.position = "fixed";
    overlay_light.style.top = "0";
    overlay_light.style.bottom = "0";
    overlay_light.style.left = "0";
    overlay_light.style.right = "0";
    overlay_light.style.opacity = "0.3";
    overlay_light.style.width = "100%";
    overlay_light.style.height = "100%";
    overlay_light.style.backgroundColor = "black";
    overlay_light.style.zIndex = "10000000003";
    overlay_light.style.zIndex = "1000003";
    overlay.onclick = closeclick;
    overlay.appendChild(overlay_light);

    var overlay_content = document.createElement("div");
    overlay_content.setAttribute("id", "visi_lightbox_content_fixed2");
    overlay_content.style.position = "fixed";
    overlay_content.style.top = "calc(50%)";
    overlay_content.style.left = "50%";
    overlay_content.style.opacity = "1";
    overlay_content.style.zIndex = "1000004";
    overlay_content.style.webkitTransform = "translate(-50%, -50%)";


    var popbox = document.createElement("div");
    popbox.setAttribute("id", "visi_lightbox_popbox2");
    popbox.style.width = width;

    var a = document.createElement("a");
    a.href = "https://www.instagram.com/wainertheswiss/";

    var img = document.createElement("img");
    img.setAttribute("id", "visi_lightbox_image");
    img.src = "https://img.visilabs.net/banner/uploaded_images/281_1070_20201009102536751.jpg";
    img.style.width = "550px";

    a.append(img);
    popbox.appendChild(a);


    var cls_btn = document.createElement("img");
    cls_btn.setAttribute("id", "img_lightbox_close");
    cls_btn.src = "data:image/png;base64,";
    cls_btn.style.position = "absolute";
    cls_btn.style.top = "-1px";
    cls_btn.style.right = "3px";
    cls_btn.style.cursor = "pointer";
    cls_btn.style.zIndex = "1000003";
    cls_btn.style.opacity = "1";
    cls_btn.onclick = closeclick;

    overlay_content.appendChild(popbox);
    overlay_content.appendChild(cls_btn);


    visi.appendChild(overlay);
    visi.appendChild(overlay_content);

    document.querySelector("body").appendChild(visi);

    function closeclick() {
        var t = document.querySelector("#VisiNotifSection");
        t.remove();
    }

    overlay_content.addEventListener("click",function(){
        <%VLSendClickFunc%>;

    });


<%VLSendImpressionFunc%>;

SetCookie(cappingName, 24);
}


function showLightBoxMobile(height, width) {

    var visi = document.createElement("div");
    visi.setAttribute("id", "VisiNotifSection");


    var overlay = document.createElement("div");
    overlay.setAttribute("id", "visi_lightbox_overlay2_wrapper2");
    overlay.style.position = "fixed";
    overlay.style.top = "0";
    overlay.style.right = "0";
    overlay.style.left = "0";
    overlay.style.bottom = "0";
    overlay.style.transform = "translate3d(0,0,3px)";


    overlay.style.zIndex = "1000003";
    var overlay_light = document.createElement("div");
    overlay_light.setAttribute("id", "visi_lightbox_overlay2");
    overlay_light.style.position = "fixed";
    overlay_light.style.top = "0";
    overlay_light.style.bottom = "0";
    overlay_light.style.left = "0";
    overlay_light.style.right = "0";
    overlay_light.style.opacity = "0.3";
    overlay_light.style.width = "100%";
    overlay_light.style.height = "100%";
    overlay_light.style.backgroundColor = "black";
    overlay_light.style.zIndex = "10000000003";
    overlay_light.style.zIndex = "1000003";
    overlay.onclick = closeclick;
    overlay.appendChild(overlay_light);

    var overlay_content = document.createElement("div");
    overlay_content.setAttribute("id", "visi_lightbox_content_fixed2");
    overlay_content.style.position = "fixed";
    overlay_content.style.top = "calc(50% - 140px)";
    overlay_content.style.left = "calc(50% - 140px)";
    overlay_content.style.opacity = "1";
    overlay_content.style.zIndex = "1000004";
    overlay_content.style.transform = "translate3d(0 , 0 , 3px)";


    var popbox = document.createElement("div");
    popbox.setAttribute("id", "visi_lightbox_popbox2");
    popbox.style.height = height;
    popbox.style.width = width;



    var a = document.createElement("a");
    a.href = "https://www.instagram.com/wainertheswiss/";

    var img = document.createElement("img");
    img.setAttribute("id", "visi_lightbox_image");
    img.src = "https://img.visilabs.net/banner/uploaded_images/281_1070_20201009102536751.jpg";
    img.style.width = "280px";

    a.append(img);
    popbox.appendChild(a);


    var cls_btn = document.createElement("img");
    cls_btn.setAttribute("id", "img_lightbox_close");
    cls_btn.src = "data:image/png;base64,";
    cls_btn.style.position = "absolute";
    cls_btn.style.top = "-1px";
    cls_btn.style.right = "3px";
    cls_btn.style.cursor = "pointer";
    cls_btn.style.zIndex = "1000003";
    cls_btn.style.width = "20px";
    cls_btn.style.opacity = "1";
    cls_btn.onclick = closeclick;

    overlay_content.appendChild(popbox);
    overlay_content.appendChild(cls_btn);


    visi.appendChild(overlay);
    visi.appendChild(overlay_content);

    document.querySelector("body").appendChild(visi);

    function closeclick() {
        var t = document.querySelector("#VisiNotifSection");
        t.remove();
    }

    overlay_content.addEventListener("click",function(){
        <%VLSendClickFunc%>;

    });

<%VLSendImpressionFunc%>;
SetCookie(cappingName, 24);
}




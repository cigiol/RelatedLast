function showLightBox(height,width){
    var visi=document.createElement("div");
    visi.setAttribute("id","VisiNotifSection");
    var overlay = document.createElement("div");
    overlay.setAttribute("id","visi_lightbox_overlay_wrapper");
    overlay.style.position="absolute";
    overlay.style.top="0";
    overlay.style.right="0";
    overlay.style.left="0";
    overlay.style.bottom="0";
    overlay.style.zIndex="1000003";
    var overlay_light = document.createElement("div"); 
    overlay_light.setAttribute("id","visi_lightbox_overlay"); 
    overlay_light.style.position="fixed"; 
    overlay_light.style.top="0";
    overlay_light.style.bottom="0"; 
    overlay_light.style.left="0";
    overlay_light.style.right="0"; 
    overlay_light.style.opacity="0.3";
    overlay_light.style.width="100%";  
    overlay_light.style.height="100%";  
    overlay_light.style.backgroundColor="black";
    overlay_light.style.zIndex="10000000003";
    overlay_light.style.zIndex="1000003";
    overlay.onclick=closeclick;
    overlay.appendChild(overlay_light);
    var overlay_content=document.createElement("div");
    overlay_content.setAttribute("id","visi_lightbox_content_fixed");
    overlay_content.style.position="fixed";
    overlay_content.style.top="50%";
    overlay_content.style.left="50%";
    overlay_content.style.opacity="1";
    overlay_content.style.zIndex="1000004";
    overlay_content.style.webkitTransform="translate(-50%, -50%)";
    var popbox = document.createElement("div");
    popbox.setAttribute("id","visi_lightbox_popbox");    
    popbox.style.height=height;
    popbox.style.width=width;
	popbox.style.background="#f7f7f7";
	var pTitle= document.createElement("p");
	pTitle.setAttribute("id","visi_lightbox_Title");
	pTitle.innerText="Fırsatlardan Haberdar Ol!";
	pTitle.style.color="#1b75bc";
	pTitle.style.fontWeight="bold";
	pTitle.style.fontSize="23px";
	pTitle.style.textAlign="center";
	pTitle.style.fontFamily="inherit";
	pTitle.style.paddingTop="10px";
	var pTextName= document.createElement("p");
	pTextName.setAttribute("class","visi_lightbox_text");
	pTextName.innerText="Adınız";
	pTextName.style.margin="10px 0 0 21px";
	var inputName= document.createElement("input");
	inputName.setAttribute("class","visi_lightbox_input");
	inputName.type="text";
    inputName.style.marginLeft="21px";
    inputName.style.width="210px";
	var pTextSurname= document.createElement("p");
	pTextSurname.setAttribute("class","visi_lightbox_text");
	pTextSurname.innerText="Soyadınız";
    pTextSurname.style.margin="10px 168px 0 0";
    pTextSurname.style.float="right";
	var inputSurname= document.createElement("input");
	inputSurname.setAttribute("class","visi_lightbox_input");
	inputSurname.type="text";
    inputSurname.style.marginRight="21px";
    inputSurname.style.float="right";
    inputSurname.style.width="210px";
    var pTextMail= document.createElement("p");
	pTextMail.setAttribute("class","visi_lightbox_text");
	pTextMail.innerText="Eposta Adresiniz";
	pTextMail.style.margin="10px 0 0 21px";
	var inputMail= document.createElement("input");
	inputMail.setAttribute("class","visi_lightbox_input");
	inputMail.type="text";
    inputMail.style.marginLeft="21px";
    inputMail.style.width="210px";
	var pTextNumber= document.createElement("p");
	pTextNumber.setAttribute("class","visi_lightbox_text");
	pTextNumber.innerText="Telefon Numaranız";
    pTextNumber.style.margin="10px 108px 0 0";
    pTextNumber.style.float="right";
	var inputNumber= document.createElement("input");
	inputNumber.setAttribute("class","visi_lightbox_input");
	inputNumber.type="text";
    inputNumber.style.marginRight="21px";
    inputNumber.style.width="210px";
    inputNumber.style.float="right";
    var pTextOptional= document.createElement("p");
	pTextOptional.setAttribute("class","visi_lightbox_text");
	pTextOptional.innerText="Lastik Ebatınız(Zorunlu Değil)";
    pTextOptional.style.margin="10px 0 0 21px";
    var br= document.createElement("br");
    var inputWidth= document.createElement("input");
	inputWidth.setAttribute("class","visi_lightbox_input");
	inputWidth.type="text";
    inputWidth.style.marginLeft="21px";
    inputWidth.style.width="138px";
    inputWidth.placeholder="Genişlik";
    var inputHeight= document.createElement("input");
	inputHeight.setAttribute("class","visi_lightbox_input");
	inputHeight.type="text";
    inputHeight.style.marginLeft="21px";
    inputHeight.style.width="138px";
    var inputRim= document.createElement("input");
	inputRim.setAttribute("class","visi_lightbox_input");
	inputRim.type="text";
	inputRim.style.marginLeft="21px";
    inputRim.style.width="138px";
    var checkBox= document.createElement("input");
    checkBox.setAttribute("id","visi_kvkk_checkbox");
    checkBox.type="checkbox";
    checkBox.style.marginTop="10px";
    checkBox.style.marginLeft="21px";
    var pTextCheck= document.createElement("p");
    pTextCheck.setAttribute("class","visi_lightbox_text");
    pTextCheck.innerText="KVKK onay alanı";
    pTextCheck.style.marginLeft="5px";
    var pTextInfo= document.createElement("p");
    pTextInfo.setAttribute("class","visi_lightbox_text");
    pTextInfo.innerText="Müşteri temsilcilerimizin size en kısa sürede dönüş yapabilmesi için güncel iletişim bilgilerinizi giriniz.";
    pTextInfo.style.marginLeft="21px";
    pTextInfo.style.textAlign="left";
    var link= document.createElement("button");
    link.innerText="Gönder";
    link.style.background="#66b014";
    link.style.cursor="pointer";
    link.style.marginLeft="21px";
    link.style.color="white";
    link.style.border="0 !important";
    var img=document.createElement("img");
    img.setAttribute("id","visi_lightbox_image");
    img.style.background="#9696b3";
    img.style.width="458px";
    img.style.height="140px"
	img.style.marginLeft="21px";
    popbox.appendChild(pTitle);
    popbox.appendChild(img);
    popbox.appendChild(pTextName);
    popbox.appendChild(pTextSurname);
    popbox.appendChild(inputName);
    popbox.appendChild(inputSurname);
    popbox.appendChild(pTextMail);
    popbox.appendChild(pTextNumber);
    popbox.appendChild(inputMail);
    popbox.appendChild(inputNumber);
    popbox.appendChild(pTextOptional);
    popbox.appendChild(br);
    popbox.appendChild(inputWidth);
    popbox.appendChild(inputHeight);
    popbox.appendChild(inputRim);
    popbox.appendChild(checkBox);
    popbox.appendChild(pTextCheck);
    popbox.appendChild(pTextInfo);
    popbox.appendChild(link);
    var cls_btn=document.createElement("img")
    cls_btn.setAttribute("id","img_lightbox_close");
    cls_btn.src="data:image/png;base64,";
    cls_btn.style.position="absolute";
    cls_btn.style.top="14px";
    cls_btn.style.right="18px";
    cls_btn.style.width="25px";
    cls_btn.style.cursor="pointer";
    cls_btn.style.zIndex="1000003";
    cls_btn.style.opacity="1";
    cls_btn.onclick=closeclick;
	var style = document.createElement("style");
    style.innerHTML = ".visi_lightbox_text{font-size:15px;text-align:center;font-family:inherit;display:inline-block;}.visi_lightbox_input{margin-top:10px;}";
    document.head.appendChild(style);
    overlay_content.appendChild(popbox);
    overlay_content.appendChild(cls_btn);
    visi.appendChild(overlay);
    visi.appendChild(overlay_content);
    document.querySelector("body").appendChild(visi);
function closeclick(){
    var t=document.querySelector("#VisiNotifSection");
    t.remove();
}
}
showLightBox("600px","500px");
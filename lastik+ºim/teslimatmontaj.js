...
BOZUK ŞUAN MOBİLDEN ÜRÜN İD ALAMIYORUM.
...
function showLightBoxDesktop(height, width) {
    ...
    img.src = "https://img.visilabs.net/banner/uploaded_images/282_1096_20200520163145377.png";
    ...
    yes.addEventListener("click", function () {
        ...
        <%VLSendClickFunc%>;
        var productId = document.querySelector("#sepetalt > div.boxBasketList.prettyScroll > div > div.itemPrice > div > div.rangeInput.itemPriceRaceInput > a:nth-child(1)").dataset.sepettekiurunid;
    ...
    yes.addEventListener("click", function () {
        ...
        for (var i = 0; i < dataLayer.length; i++) {
            if (dataLayer[i].event == "checkout") {
                productName=dataLayer[i].ecommerce.checkout.products[0].id;
            }
        }
        kayitliHizmetChange('t4_set_mobil', 389, productId);
        closeclick();
...
for (var i = 0; i < dataLayer.length; i++) {
    if (dataLayer[i].event == "checkout") {
        productName=dataLayer[i].ecommerce.checkout.products[0].name;
    }
}
...
var _interval = setInterval(function () {
    try {
        ...
        for (var i = 0; i < dataLayer.length; i++) {
            if (dataLayer[i].event == "changeLocation") {
                if (dataLayer[i].locationPlate == "34") {
                    if (parseInt(document.querySelector("#sepetmiktari").innerText) > 3 && document.querySelector(".toggle").style.display=="none") {
                        if (!productName.includes("R12") && !productName.includes("R13") && !productName.includes("R14")) {

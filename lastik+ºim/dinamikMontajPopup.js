function showLightBoxDesktop(height, width) {

    var visi = document.createElement("div");
    visi.setAttribute("id", "VisiNotifSection34");


    var overlay = document.createElement("div");
    overlay.setAttribute("id", "visi_lightbox_overlay_wrapper");
    overlay.style.position = "absolute";
    overlay.style.top = "0";
    overlay.style.right = "0";
    overlay.style.left = "0";
    overlay.style.bottom = "0";


    overlay.style.zIndex = "1000003";
    var overlay_light = document.createElement("div");
    overlay_light.setAttribute("id", "visi_lightbox_overlay");
    overlay_light.style.position = "fixed";
    overlay_light.style.top = "0";
    overlay_light.style.bottom = "0";
    overlay_light.style.left = "0";
    overlay_light.style.right = "0";
    overlay_light.style.opacity = "0.3";
    overlay_light.style.width = "100%";
    overlay_light.style.height = "100%";
    overlay_light.style.backgroundColor = "black";
    overlay_light.style.zIndex = "10000000003";
    overlay_light.style.zIndex = "1000003";
    overlay.onclick = closeclick;
    overlay.appendChild(overlay_light);

    var overlay_content = document.createElement("div");
    overlay_content.setAttribute("id", "visi_lightbox_content_fixed");
    overlay_content.style.position = "fixed";
    overlay_content.style.top = "calc(50%)";
    overlay_content.style.left = "50%";
    overlay_content.style.opacity = "1";
    overlay_content.style.zIndex = "1000004";
    overlay_content.style.webkitTransform = "translate(-50%, -50%)";


    var popbox = document.createElement("div");
    popbox.setAttribute("id", "visi_lightbox_popbox");
    popbox.style.height = "100%";
    popbox.style.width = "auto";


    var price = 99;
    var services = document.querySelectorAll('#pophizmetler > div > div > div.hizmetler > table > tbody > tr');
    for (var i = 1; i < services.length; i++) {

        if (document.querySelectorAll('#pophizmetler > div > div > div.hizmetler > table > tbody > tr')[i].querySelector('td:nth-child(1)').innerText.includes("Montajı")) {
            if (parseInt(document.querySelectorAll('#pophizmetler > div > div > div.hizmetler > table > tbody > tr')[i].querySelector('td:nth-child(4)').innerText.split(" ")[0]) < 100) {
                price = 99 - parseInt(document.querySelectorAll('#pophizmetler > div > div > div.hizmetler > table > tbody > tr')[i].querySelector('td:nth-child(4)').innerText.split(" ")[0]);
            }

        }

    }


    var priceText = document.createElement("h1");
    priceText.innerText = price;
    priceText.style = "position: absolute;\
    left: 200px;\
    color: white;\
    font-weight: bold;\
    font-size: 30px;\
    top: 6px;";
    popbox.appendChild(priceText);

    var img = document.createElement("img");
    img.setAttribute("id", "visi_lightbox_image");
    img.src = "https://img.visilabs.net/banner/uploaded_images/282_1096_20200518113702897.png";
    img.style.width = "auto";
    img.style.width = "100%";
    popbox.appendChild(img);

    var yes = document.createElement("img");
    yes.setAttribute("id", "visi_lightbox_yes");
    yes.src = "https://img.visilabs.net/banner/uploaded_images/282_1096_20200518113741865.png";
    yes.style = "width: 160px;\
    position: absolute;\
    bottom: 10px;\
    left: 45px;\
    cursor: pointer;";
    popbox.appendChild(yes);

    var no = document.createElement("img");
    no.setAttribute("id", "visi_lightbox_no");
    no.src = "https://img.visilabs.net/banner/uploaded_images/282_1096_20200518113723379.png";
    no.style = "width: 160px;\
    position: absolute;\
    bottom: 10px;\
    right: 45px;\
    cursor: pointer;";
    popbox.appendChild(no);


    var cls_btn = document.createElement("img");
    cls_btn.setAttribute("id", "img_lightbox_close");
    cls_btn.src = "data:image/png;base64,";
    cls_btn.style.position = "absolute";
    cls_btn.style.top = "-1px";
    cls_btn.style.right = "3px";
    cls_btn.style.cursor = "pointer";
    cls_btn.style.zIndex = "1000003";
    cls_btn.style.opacity = "1";
    cls_btn.onclick = closeclick;

    overlay_content.appendChild(popbox);
    overlay_content.appendChild(cls_btn);


    visi.appendChild(overlay);
    visi.appendChild(overlay_content);


    document.querySelector("body").appendChild(visi);

    function closeclick() {
        var t = document.querySelector("#VisiNotifSection34");
        t.remove();
    }

    yes.addEventListener("click", function () {
    
            <%VLSendClickFunc%>;
        var productId = parseInt(document.querySelector(".sepetCard").id.split("_")[1]);
        kayitliHizmetChange('t4_set_mobil', 389, productId);

        closeclick();

    });


    no.addEventListener("click", function () {

        closeclick();

    });
    
    <%VLSendImpressionFunc%>;



}
function showLightBoxMobile(height, width) {

    var visi = document.createElement("div");
    visi.setAttribute("id", "VisiNotifSection34");


    var overlay = document.createElement("div");
    overlay.setAttribute("id", "visi_lightbox_overlay_wrapper");
    overlay.style.position = "absolute";
    overlay.style.top = "0";
    overlay.style.right = "0";
    overlay.style.left = "0";
    overlay.style.bottom = "0";


    overlay.style.zIndex = "1000003";
    var overlay_light = document.createElement("div");
    overlay_light.setAttribute("id", "visi_lightbox_overlay");
    overlay_light.style.position = "fixed";
    overlay_light.style.top = "0";
    overlay_light.style.bottom = "0";
    overlay_light.style.left = "0";
    overlay_light.style.right = "0";
    overlay_light.style.opacity = "0.3";
    overlay_light.style.width = "100%";
    overlay_light.style.height = "100%";
    overlay_light.style.backgroundColor = "black";
    overlay_light.style.zIndex = "10000000003";
    overlay_light.style.zIndex = "1000003";
    overlay.onclick = closeclick;
    overlay.appendChild(overlay_light);

    var overlay_content = document.createElement("div");
    overlay_content.setAttribute("id", "visi_lightbox_content_fixed");
    overlay_content.style.position = "fixed";
    overlay_content.style.top = "calc(50%)";
    overlay_content.style.left = "50%";
    overlay_content.style.opacity = "1";
    overlay_content.style.zIndex = "1000004";
    overlay_content.style.width="300px";
    overlay_content.style.webkitTransform = "translate(-50%, -50%)";


    var popbox = document.createElement("div");
    popbox.setAttribute("id", "visi_lightbox_popbox");
    popbox.style.height = "100%";
    popbox.style.width = "auto";


    var price = 99;
    var services = document.querySelectorAll('#pophizmetler > div > div > div.hizmetler > table > tbody > tr');
    for (var i = 1; i < services.length; i++) {

        if (document.querySelectorAll('#pophizmetler > div > div > div.hizmetler > table > tbody > tr')[i].querySelector('td:nth-child(1)').innerText.includes("Montajı")) {
            if (parseInt(document.querySelectorAll('#pophizmetler > div > div > div.hizmetler > table > tbody > tr')[i].querySelector('td:nth-child(4)').innerText.split(" ")[0]) < 100) {
                price = 99 - parseInt(document.querySelectorAll('#pophizmetler > div > div > div.hizmetler > table > tbody > tr')[i].querySelector('td:nth-child(4)').innerText.split(" ")[0]);
            }

        }

    }


    var priceText = document.createElement("h1");
    priceText.innerText = price;
    priceText.style = "position: absolute;\
    left: 136px;\
    color: white;\
    font-weight: bold;\
    font-size: 20px;\
    top: 6px;";
    popbox.appendChild(priceText);

    var img = document.createElement("img");
    img.setAttribute("id", "visi_lightbox_image");
    img.src = "https://img.visilabs.net/banner/uploaded_images/282_1096_20200518113702897.png";
    img.style.width = "auto";
    img.style.width = "100%";
    popbox.appendChild(img);

    var yes = document.createElement("img");
    yes.setAttribute("id", "visi_lightbox_yes");
    yes.src = "https://img.visilabs.net/banner/uploaded_images/282_1096_20200518113741865.png";
    yes.style = "width: 110px;\
    position: absolute;\
    bottom: 10px;\
    left: 25px;\
    cursor: pointer;";
    popbox.appendChild(yes);

    var no = document.createElement("img");
    no.setAttribute("id", "visi_lightbox_no");
    no.src = "https://img.visilabs.net/banner/uploaded_images/282_1096_20200518113723379.png";
    no.style = "width: 110px;\
    position: absolute;\
    bottom: 10px;\
    right: 25px;\
    cursor: pointer;";
    popbox.appendChild(no);


    var cls_btn = document.createElement("img");
    cls_btn.setAttribute("id", "img_lightbox_close");
    cls_btn.src = "data:image/png;base64,";
    cls_btn.style.position = "absolute";
    cls_btn.style.top = "-1px";
    cls_btn.style.right = "3px";
    cls_btn.style.cursor = "pointer";
    cls_btn.style.zIndex = "1000003";
    cls_btn.style.opacity = "1";
    cls_btn.style.width="25px";
    cls_btn.onclick = closeclick;

    overlay_content.appendChild(popbox);
    overlay_content.appendChild(cls_btn);


    visi.appendChild(overlay);
    visi.appendChild(overlay_content);


    document.querySelector("body").appendChild(visi);

    function closeclick() {
        var t = document.querySelector("#VisiNotifSection34");
        t.remove();
    }

    yes.addEventListener("click", function () {
    
            <%VLSendClickFunc%>;
        var productId = parseInt(document.querySelector(".sepetCard").id.split("_")[1]);
        kayitliHizmetChange('t4_set_mobil', 389, productId);

        closeclick();

    });


    no.addEventListener("click", function () {

        closeclick();

    });
    
    <%VLSendImpressionFunc%>;



}
var exist = false;




if (!document.querySelector("#VisiNotifSection34")) {
    if (document.querySelectorAll('#pophizmetler > div > div > div.hizmetler > table > tbody > tr').length > 1) {
        if (document.querySelectorAll('#pophizmetler > div > div > div.hizmetler > table > tbody > tr')[1].querySelector('td:nth-child(1)').innerText.split(" ")[1] == "Mobil") {
            exist = true;
        }

    }
    if (document.querySelector(".cardTitle.cursor-pointer")) {
        if (!exist && !document.querySelector(".cardTitle.cursor-pointer").innerText.includes("R12") && !document.querySelector(".cardTitle.cursor-pointer").innerText.includes("R13") && !document.querySelector(".cardTitle.cursor-pointer").innerText.includes("R14")) {
            const metas = document.getElementsByTagName('meta');
            for (let i = 0; i < metas.length; i++) {
                if (metas[i].getAttribute('name') === "last_firm_location") {
                    if (metas[i].getAttribute("content") == "34") {
                        if (parseInt(document.querySelector("#sepetmiktari").innerText) > 3) {
                            if(window.innerWidth>500){
                                showLightBoxDesktop("270px", "550px");
                            }
                            else{
                                showLightBoxMobile("270px", "550px");
                            }
                            
                            
                        }
                    }
                }
            }

        }
    }

}

